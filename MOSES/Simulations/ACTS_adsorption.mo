...
model ACTS_adsorption
  ...
  parameter Real L=0.2514;                             //length                                                                   [m]
  ...
  //boundary conditions
  parameter Real T2(unit="K",displayUnit="degC")=10+273.15;     //temperature boundary condition in the fluid condenser/evaporator side    [K]  desorption T2=38.8°C, adsorption T2=10°C, note: it is not necessarily the saturation temperature (can be superheated)
  parameter Real Psat_theo(displayUnit="Pa")=1166;                   //saturation pressure boundary condition in the condenser/evaporator side  [Pa] desorption Psat=6283Pa (slight overheat) (Tamb=°C?), adsorption Psat=1166Pa (slight overheat) (Tamb=°C?)

  //initial conditions
  parameter Real Ti(unit="K",displayUnit="degC")=357.91;  //initial temperature of the particles                                     [K]
  parameter Real Wi=0.063243;                                     //initial loading state of the particles                                   [-]

  //inlet conditions
  parameter Real T_h_in(unit="K",displayUnit="degC")=25+273.15; //inlet temperature of heat transfer fluid                                 [K] desorption Th_in=90°C, Th_in=adsorption=25°C
  ...
  parameter Real mdot_h=0.001012;                                 //inlet heat transfer fluid mass flow rate                                 [kg/s]
